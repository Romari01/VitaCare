...
export default function Asistentes() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      handleClose()
      fetchAsistentes()
      showToast(editingId ? 'Asistente actualizado correctamente' : 'Asistente registrado correctamente', 'success')
    } catch (error) {
                ...
                <tr key={a._id} className={`transition-colors ${darkMode ? 'hover:bg-gray-700/50' : 'hover:bg-slate-50'}`}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-sm font-medium whitespace-nowrap ${darkMode ? 'text-white' : 'text-slate-800'}`}>{a.name}</span>
                    </div>
                  </td>
                  <td className={`px-6 py-4 text-sm whitespace-nowrap ${darkMode ? 'text-gray-300' : 'text-slate-600'}`}>{a.email}</td>
                  <td className={`px-6 py-4 text-sm whitespace-nowrap ${darkMode ? 'text-gray-300' : 'text-slate-600'}`}>{a.phone || '—'}</td>
                  <td className={`px-6 py-4 text-sm whitespace-nowrap ${darkMode ? 'text-gray-300' : 'text-slate-600'}`}>{a.dni || '—'}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
