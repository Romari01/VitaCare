...
export default function Usuarios() {
  ...
  const rolColor = (role) => {
    if (role === 'admin') return 'bg-purple-50 text-purple-600 border border-purple-200'
    if (role === 'doctor') return 'bg-blue-50 text-blue-600 border border-blue-200'
    if (role === 'admision') return 'bg-teal-50 text-teal-600 border border-teal-200'
  ...
  return (
    <div className={`p-6 min-h-full transition-colors ${darkMode ? 'bg-gray-900' : 'bg-slate-50'}`}>
      ...
      {/* Stats roles — solo personal */}
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Administradores', role: 'admin', icon: '🔑', color: 'bg-purple-50 text-purple-600' },
          { label: 'Doctores', role: 'doctor', icon: '👨‍⚕️', color: 'bg-blue-50 text-blue-600' },
          { label: 'Admisión', role: 'admision', icon: '🖥️', color: 'bg-teal-50 text-teal-600' },
        ].map((s) => (
          <div key={s.role}
      ...
      <div className={`rounded-2xl border overflow-hidden shadow-sm ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-slate-100'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="text" value={search}
              ...
              className={`border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400 ${
                darkMode ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' : 'border-slate-200'
              }`}
            />
          </div>
        </div>
        ...
        <table className="w-full">
          <thead className={`border-b ${darkMode ? 'bg-gray-700/50 border-gray-700' : 'bg-slate-50 border-slate-100'}`}>
            <tr>
              {['Nro', 'Nombre', 'Email', 'Rol', 'DNI', 'Teléfono', 'Acciones'].map(h => (
              ...
              <tr key={u._id} className={`transition-colors ${darkMode ? 'hover:bg-gray-700/50' : 'hover:bg-slate-50'}`}>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`text-sm font-medium ${darkMode ? 'text-white' : 'text-slate-800'}`}>{u.name}</span>
                  </div>
                </td>
                <td className={`px-6 py-4 text-sm ${darkMode ? 'text-gray-300' : 'text-slate-600'}`}>{u.email}</td>
                <td className="px-6 py-4">
                  <span className={`text-xs px-2 py-1 rounded-full font-semibold capitalize ${rolColor(u.role)}`}>
                    {u.role}
                  </span>
                </td>
                <td className={`px-6 py-4 text-sm ${darkMode ? 'text-gray-300' : 'text-slate-600'}`}>{u.dni || '—'}</td>
                <td className={`px-6 py-4 text-sm ${darkMode ? 'text-gray-300' : 'text-slate-600'}`}>{u.phone || '—'}</td>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
